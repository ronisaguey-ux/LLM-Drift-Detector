#!/usr/bin/env bash
# install_global_hooks.sh — Installs DriftClean global environment hooks for Node.js and Python
set -euo pipefail
...
echo "⚡ Installing DriftClean Global AI Toolchain Hooks..."
...
# 2. Configure shell profiles (~/.bashrc, ~/.profile, ~/.zshrc)
add_env_to_file() {
    local target_file="$1"
...
# <<< DRIFT_CLEAN_HOOKS <<<
EOF
            echo "✅ Configured hooks in: $target_file"
        else
...
echo "🎉 Global DriftClean hooks installed successfully!"
